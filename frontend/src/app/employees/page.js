...
import { Users, Search, CheckCircle, Clock, ShieldCheck, UserPlus, Filter, ChevronLeft, ChevronRight, AlertCircle, Building2, Briefcase } from 'lucide-react';
...
export default function EmployeesPage() {
  ...
  const isManager = isSystemAdmin || userRoleName === 'MANAGER' || currentUser?.RoleId === 1;
  ...
  const loadEmployees = useCallback(async () => {
    ...
    // If Manager (and not SysAdmin), restrict to employees under their ManagerId or RoleId=2
    const params = {
      pageNumber,
      pageSize,
      search,
    ...
    if (!isSystemAdmin && currentUser?.UserId) {
      params.managerId = currentUser.UserId;
    }
  ...
  const filteredItems = (pagedData?.items || []).filter(u => {
    // Managers can strictly ONLY see employees reporting to them
    if (!isSystemAdmin && u.ManagerId !== currentUser?.UserId) {
      return false;
    }
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-dark, #0f172a)', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Users size={28} style={{ color: 'var(--primary, #3b82f6)' }} />
            <span>{isSystemAdmin ? 'All Employees' : 'My Team Employees'}</span>
          </h1>
          <p style={{ color: '#64748b', fontSize: '0.9rem', margin: '4px 0 0 0' }}>
            {isSystemAdmin 
              ? 'Manage and review employee accounts across all teams and departments.'
              : `View and manage team employees reporting to ${currentUser?.FirstName} ${currentUser?.LastName}.`}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <div style={{ background: '#ffffff', borderRadius: '12px', padding: '16px 20px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', tracking: '0.05em' }}>Total Employees</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#0f172a', marginTop: '4px' }}>{totalCount}</div>
        </div>

        <div style={{ background: '#ffffff', borderRadius: '12px', padding: '16px 20px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#047857', textTransform: 'uppercase' }}>Active Employees</div>
      ...
      <div style={{ background: '#ffffff', borderRadius: '12px', padding: '16px 20px', border: '1px solid #e2e8f0', marginBottom: '24px', display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1', minWidth: '260px' }}>
          ...
          <input
            type="text"
            placeholder="Search by name, email, company, team..."
            value={search}
      ...
      {/* Cards List Grid */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '48px', color: '#64748b' }}>Loading employees data...</div>
        ...
        <div style={{ textAlign: 'center', padding: '48px', background: '#ffffff', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
          ...
          <h3 style={{ fontSize: '1.1rem', color: '#334155', margin: '0 0 4px 0' }}>No employees found</h3>
          <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>Try changing your search terms or filter selection.</p>
        </div>
      ) : (
            ...
            >
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '14px' }}>
                  <img
                    src={user.ProfilePicturePath?.startsWith('/uploads/') ? `http://localhost:5148${user.ProfilePicturePath}` : (user.ProfilePicturePath || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=facearea&facepad=2&w=256&h=256&q=80')}
              ...
              <div style={{ paddingTop: '12px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                {!user.IsActive && (
                  ...
                  >
                    <ShieldCheck size={15} />
                    {actionLoading === user.UserId ? 'Approving...' : 'Approve Account'}
                  </button>
                )}
                <button
